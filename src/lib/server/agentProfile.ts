...
async function buildAgentProfilePayload(profile: ResolvedProfile, origin: string) {
  ...
  const historyWhere = profile.historyByOwner && user ? { ownerUserId: user.id } : { agentId: agent.id };
  ...
  const profileContent = user
    ? await prisma.agentProfileContent.findFirst({
        ...
        where: {
          ownerUserId: user.id,
          OR: [{ agentId: agent.id }, { agentId: profile.profileId }],
        },
      })
    : undefined;
...
async function resolveProfileById(profileId: string, normalizedAgentId: string | undefined): Promise<ResolvedProfile | undefined> {
  ...
  const activeAgent = normalizedAgentId
    ? agents.find((item) => item.id === normalizedAgentId) ?? agents.find((item) => item.ownerUserId === profileId)
    : agents.find((item) => item.ownerUserId === profileId);
  if (activeAgent) {
    ...
    return {
      agent: activeAgent,
      historyByOwner: user?.id === profileId,
      live: true,
      profileId,
  ...
  const ownerProfile = await resolveProfileByOwner(profileId);
  if (ownerProfile) {
    return ownerProfile;
  }

  const userWithoutAgent = await prisma.user.findUnique({ where: { id: profileId } });
  if (userWithoutAgent) {
    return {
      agent: agentFromStoredProfile({
        agentId: userWithoutAgent.id,
        agentName: userWithoutAgent.name,
        ownerUserId: userWithoutAgent.id,
        registeredAt: userWithoutAgent.createdAt,
      }),
      historyByOwner: true,
      live: false,
      profileId,
...
async function resolveProfileByOwner(ownerUserId: string): Promise<ResolvedProfile | undefined> {
  const activeAgent = listAgents().find((item) => item.ownerUserId === ownerUserId);
  if (activeAgent) {
    const [user, qualification] = await Promise.all([
...
function safeNormalizeAgentId(value: string) {
  try {
